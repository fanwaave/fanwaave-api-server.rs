...
pub struct CliEnvValues {
    ...
    pub fanwaave_api_bind: Option<String>,
    pub fanwaave_api_tcp_bind: Option<String>,
    pub fanwaave_nats_url: Option<String>,
...
pub fn load_from(lookup: impl Fn(&str) -> Option<String>) -> CliEnvValues {
    CliEnvValues {
        ...
        fanwaave_api_bind: lookup("FANWAAVE_API_BIND").filter(|value| !value.is_empty()),
        fanwaave_api_tcp_bind: lookup("FANWAAVE_API_TCP_BIND").filter(|value| !value.is_empty()),
        fanwaave_nats_url: lookup("FANWAAVE_NATS_URL").filter(|value| !value.is_empty()),
...
pub fn load_env_map(
    ...
    let fanwaave_api_bind = pick(&["FANWAAVE_API_BIND"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = fanwaave_api_bind {
        out.insert("FANWAAVE_API_BIND".to_string(), value);
    }
    let fanwaave_api_tcp_bind = pick(&["FANWAAVE_API_TCP_BIND"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = fanwaave_api_tcp_bind {
        out.insert("FANWAAVE_API_TCP_BIND".to_string(), value);
    }
    let fanwaave_nats_url = pick(&["FANWAAVE_NATS_URL"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = fanwaave_nats_url {
        out.insert("FANWAAVE_NATS_URL".to_string(), value);
    }
