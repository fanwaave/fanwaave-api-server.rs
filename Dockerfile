...
#
# Multi-stage image for fanwaave-api-server.
# Prefer linux/arm64:
#   docker buildx build --platform linux/arm64 -t fanwaave-api-server:dev .
#   docker run --rm --platform linux/arm64 \
#     -e SOPS_AGE_KEY="$(cat ~/.config/sops/age/keys.txt)" fanwaave-api-server:dev
#
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,id=cargo-registry,sharing=locked \
    ...
    --mount=type=cache,target=/src/target,id=fanwaave-api-server-target-${TARGETARCH},sharing=locked \
...
COPY --from=build "/usr/local/bin/fanwaave-api-server" "/usr/local/bin/fanwaave-api-server"
...
RUN --mount=type=bind,source=.,target=/src,ro \
    ...
    && chown -R 65532:65532 /app /usr/local/bin/fanwaave-api-server
ENV SOPS_SECRETS_FILE=/app/secrets/app.env \
    OTEL_SERVICE_NAME=fanwaave-api-server \
