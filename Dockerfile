...
#
# fanwaave-api-server — production container image.
#
...
#   docker buildx build --platform linux/arm64 -t ghcr.io/fanwaave/fanwaave-api-server:dev --load .
#
...
#     -t ghcr.io/fanwaave/fanwaave-api-server:dev --load .
#
#   # OCI image index with provenance and SBOM attestations:
#   docker buildx build --platform linux/arm64 --push \
#     --provenance=true --sbom=true \
#     --output type=image,oci-mediatypes=true,name=ghcr.io/fanwaave/fanwaave-api-server:dev .
#
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,id=cargo-registry,sharing=locked \
    ...
    cargo build --release --bin fanwaave-api-server; \
    strip target/release/fanwaave-api-server; \
    cp target/release/fanwaave-api-server /usr/local/bin/fanwaave-api-server
...
COPY --from=builder --chown=65532:65532 /usr/local/bin/fanwaave-api-server /usr/local/bin/fanwaave-api-server
ENV FANWAAVE_API_BIND=0.0.0.0:8080
EXPOSE 8080
USER 65532:65532
ENTRYPOINT ["/usr/local/bin/fanwaave-api-server"]
...
COPY --from=builder /usr/local/bin/fanwaave-api-server /usr/local/bin/fanwaave-api-server
...
#     ghcr.io/fanwaave/fanwaave-api-server:dev
#
